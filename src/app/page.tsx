...
// APIから返されるデータの型定義
type GroupedTools = Record<string, Tool[]>;
  ...
  useEffect(() => {
    const fetchTools = async () => {
      try {
        ...
        const data: GroupedTools = await response.json();
          ...
          {Object.entries(groupedTools).map(([category, tools]) => (
            <section key={category}>
              <motion.h2 
                ...
                transition={{ duration: 0.5 }}
                className="text-2xl font-bold text-slate-700 mb-6 border-b-2 border-pink-200 pb-2"
              >
                {category}
              </motion.h2>
              <motion.div 
